...
class AdmissionScoringService
{
    ...
    {
        $key = DB::table('admission_answer_keys')->where('admission_cycle_id', $cycle->id)->pluck('correct_answer', 'item_number');
        ...
        foreach ($cycle->applicants()->whereNotNull('submitted_at')->get() as $applicant) {
            ...
            $percent = $correct / $key->count() * 100;
            $applicant->forceFill(['exam_score' => $correct, 'stanine_score' => min(9, max(1, (int) ceil($percent / (100 / 9))))])->save();
        }
        $this->evaluate($cycle);
    }
    ...
    public function submit(AdmissionApplicant $applicant, array $answers): void
    {
        DB::transaction(function () use ($applicant, $answers): void {
            ...
            $applicant->forceFill([
                'answers' => $answers,
                'exam_score' => $correct,
                'stanine_score' => min(9, max(1, (int) ceil($percent / (100 / 9)))),
                'submitted_at' => now(),
                'exam_token' => null,
            ])->save();
    ...
    {
        $quotas = DB::table('admission_course_quotas')->where('admission_cycle_id', $cycle->id)->pluck('seats', 'course_code');
        $itemCount = max(1, DB::table('admission_answer_keys')->where('admission_cycle_id', $cycle->id)->count());
        foreach ($cycle->applicants()->get()->groupBy('course_choice') as $course => $group) {
            $ranked = $group->map(function (AdmissionApplicant $applicant) use ($cycle, $itemCount): AdmissionApplicant {
                $total = $applicant->exam_score === null || $applicant->gwa === null || $applicant->interview_score === null
                    ? null
                        : round((float) $applicant->exam_score / $itemCount * $cycle->exam_weight
                        + (float) $applicant->gwa * $cycle->gwa_weight / 100
                        + (float) $applicant->interview_score * $cycle->interview_weight / 100, 2);
