(() => {
    document.querySelectorAll('[data-live-queue]').forEach(queue => {
        ...
        async function refresh(next = url, manual = false) {
            ...
            try {
                const response = await fetch(url, { headers: { Accept: queue.dataset.json === 'true' ? 'application/json' : 'text/html' }, cache: 'no-store', signal: controller.signal });
        ...
        queue.addEventListener('submit', event => {
            ...
            if (event.target.id === 'archive-selection' && !results.querySelector('input[name="ids[]"]:checked, [data-security-termination][open]')) {
