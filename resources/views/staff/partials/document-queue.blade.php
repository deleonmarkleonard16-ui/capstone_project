    ...
    <div class="table-responsive"><table class="table align-middle" @if($mode === 'queue') data-individual-queue data-module="{{ $moduleKey }}" @endif>
        ...
        @forelse($requests as $entry)
            <tr data-request-id="{{ $entry->id }}">
                <td><span class="badge text-bg-primary mb-2">{{ $entry->reference }}</span><br><strong>{{ mb_strtoupper(trim($entry->first_name.' '.($entry->middle_name ? $entry->middle_name.' ' : '').$entry->last_name)) }}</strong><div class="small text-muted">{{ $entry->student_number ?: 'Alumni' }} · {{ $entry->created_at->timezone('Asia/Manila')->format('M d, Y') }}</div></td>
                ...
                <td>@include('staff.partials.document-status')</td>
                <td>@if($entry->proof_path)<button type="button" class="btn btn-link btn-sm p-0" data-receipt="{{ route(auth()->user()->role.'.documents.proof', $entry) }}">Show receipt</button>@else<span class="text-muted">Not uploaded</span>@endif</td>
                <td class="text-nowrap">@include('staff.partials.document-actions')
                    @if(in_array($entry->status, ['ready', 'completed'], true))<button type="button" class="btn btn-sm btn-outline-success" data-document-print="document-print-{{ $entry->id }}">{{ $moduleKey === 'good-moral' ? 'Print Certificate' : 'Print Exit Clearance Slip' }}</button>@endif
                </td>
            </tr>
        @empty
